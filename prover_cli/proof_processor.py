...
def execute_task(witness_file, previous_proof=None):
    ...
    if previous_proof:
        command = f"""
        env RUST_BACKTRACE=full RUST_LOG=debug leader --runtime=amqp --amqp-uri={AMQP_URI} stdio --previous-proof {previous_proof} < {witness_file} | tee {output_file}
        """
    else:
        command = f"""
        env RUST_BACKTRACE=full RUST_LOG=debug leader --runtime=amqp --amqp-uri={AMQP_URI} stdio < {witness_file} | tee {output_file}
        """
...
def log_metrics_to_csv(witness_file, metrics, start_time, end_time):
    ...
    with open(csv_file_path, mode='a', newline='') as file:
        ...
        if not file_exists:
            writer.writerow(['block number', 'metric name',
                            'start_time', 'end_time', 'data point values'])
...
def validate_and_extract_proof(raw_json):
    try:
        ...
        result = subprocess.run(
            ['sh', '-c', f'echo \'{raw_json}\' | jq .'], capture_output=True, text=True)
        if result.returncode == 0:
            proof_json = json.loads(result.stdout)
